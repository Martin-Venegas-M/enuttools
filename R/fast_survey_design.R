#' Quickly create a survey design object from raw data
#'
...
#' @param ids Variable indicating the primary sampling units (PSUs).
#'   Defaults to \code{"varunit"}. Can be provided unquoted using tidy evaluation.
#' @param strata Variable indicating strata identifiers.
#'   Defaults to \code{"varstrat"}. Can also be provided unquoted using tidy evaluation.
#' @param pop Logical expression (tidy-evaluation style) defining the subpopulation
#'   to include in the design. Defaults to \code{TRUE} (include all observations).
#' @param transform.missings Logical; if \code{TRUE}, recodes all occurrences of
#'   \code{missing.code} as \code{NA} in the columns specified by \code{cols.transform}.
#'   Default is \code{TRUE}.
#' @param cols.transform Column selection (tidyselect syntax) to apply missing
#'   transformation. Defaults to \code{everything()}.
#' @param missing.code Numeric or character value used in the dataset to represent
#'   missing values. Default is \code{-777}.
#' @param ... Additional arguments passed to \code{srvyr::as_survey_design()}, such as
...
#' }
#'
#' @seealso \code{\link[srvyr]{as_survey_design}}, \code{\link[dplyr]{mutate}},
#'   \code{\link[dplyr]{across}}, \code{\link[dplyr]{filter}}.
#'
#' @importFrom srvyr as_survey_design
#' @importFrom dplyr mutate across filter everything if_else
#' @export

fast_survey_design <- function(
  data,
  ids = "varunit",
  strata = "varstrat",
  pop = TRUE,
  ...
  # Transform missing code to NA if user wants it
  if (transform.missings) {
    data <- data %>%
      mutate(across(cols.transform, ~ if_else(. == missing.code, NA, .)))
  }

  # Filter by population to analyse
  data <- data %>%
    dplyr::filter({{ pop }})
